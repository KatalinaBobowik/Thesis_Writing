...
% WeperformedaGOtermenrichmentonthetopdifferentiallyexpressedgenesbetweenMtlandSag (lfc > 0.65 and p-value < 0.05/15632, n=460).
\label{fig:Revigo}
\end{figure}
...
\begin{table}[htb!]
\centering
\caption{Batch and replicate information for all 117 samples (plus 6 technical replicates) used in the RNA-Seq analysis. (See supplementary file associated with this dissertation.)
}
\label{table: Batch and replicate information}
\end{table}

\begin{table}[htb!]
\centering
\caption{Total number of reads at each step of the pipeline. (See supplementary file associated with this dissertation.)
}
\label{table: nReads all stages}
\end{table}

\begin{table}[htb!]
\centering
\caption{Full list of covariates analysed in the RNA-seq analysis. (See supplementary file associated with this dissertation.)
}
\label{table: all Covariates}
\end{table}

\begin{table}[htb!]
\centering
\caption{Associations between covariates and all PCA dimensions of the log2-transformed, TMM-normalised count data. (See supplementary file associated with this dissertation.)
}
\label{table: PCA assoc}
\end{table}

\begin{table}[htb!]
\centering
\caption{Weights from RUVs-corrected output. (See supplementary file associated with this dissertation.)
}
\label{table: RUVs Weights}
\end{table}

\begin{table}[htb!]
\centering
\caption{Limma-corrected comparisons for each PCA. (See supplementary file associated with this dissertation.)
}
\label{table: Limma-corrected PCA assoc}
\end{table}

\begin{table}[htb!]
\centering
\caption{RUVs-corrected comparisons for each PCA. (See supplementary file associated with this dissertation.)
}
\label{table: RUVs-corrected PCA assoc}
\end{table}
